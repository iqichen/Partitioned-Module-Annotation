...
# 参数解析
parser = argparse.ArgumentParser(description='使用K-means聚类拆分MaskTrack RCNN模型')
...
# 初始化配置（需确保配置文件正确）
config_file = r'E:\Code\Partitioned Module Annotation\mmdetection\configs\masktrack_rcnn\masktrack-rcnn_mask-rcnn_r50_fpn_8xb1-12e_youtubevis2019.py'
checkpoint_file = r'E:\Code\Partitioned Module Annotation\mmdetection\checkpoints\masktrack_rcnn_r50_fpn_12e_youtubevis2019_20211022_194830-6ca6b91e.pth'
img_folder = r'E:\Code\Partitioned Module Annotation\data\coco\val2017'
...
class BatchFeatureHook:
    ...
    def hook_fn(self, module, input, output):
        if isinstance(output, tuple):
            output = output[0]
        feat = output.detach().cpu().numpy()  # [B, C, H, W]
...
if 'detector' in cfg.model:  # 适配视频模型结构
    cfg.model.detector.backbone.type = 'ResNet'
    cfg.model.detector.backbone.depth = 50

# 初始化模型
model = init_detector(cfg, checkpoint_file, device='cuda:0')

# 关键修复2：正确访问backbone（视频模型的嵌套结构）
...
# 获取模型中的所有卷积层
all_conv_layers = get_all_conv_layers(backbone)
...
for block_key, layer_names in block_groups.items():
    # 优先选择conv3作为代表（ResNet50的Bottleneck结构）
...
def prepare_image(img):
    """适配视频模型的预处理"""
    img = mmcv.imrescale(img, (1333, 800))  # YouTube-VIS标准输入尺寸
...
with torch.no_grad():
    for img in tqdm(processed_imgs, desc='特征提取'):
        # 输入格式：[num_frames, batch, C, H, W]
        video_input = img.unsqueeze(0)  # 模拟单帧视频
        _ = backbone(video_input)  # 直接使用backbone避免跟踪逻辑
...
# 保存中间数据
output_file = os.path.join(SAVE_DIR, f'masktrack_rcnn_features_{args.k}.pkl')
