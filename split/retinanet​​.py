...
# 参数解析
parser = argparse.ArgumentParser(description='使用K-means聚类拆分RetinaNet模型')
...
# 初始化配置
config_file = r'E:\Code\Partitioned Module Annotation\mmdetection\configs\retinanet\retinanet_r50_fpn_1x_coco.py'
checkpoint_file = r'E:\Code\Partitioned Module Annotation\mmdetection\checkpoints\retinanet_r50_fpn_1x_coco_20200130-c2398f9e.pth'
...
for layer_name, layer in selected_layers.items():
    try:
        ...
        print("建议执行 print(model.backbone) 查看实际结构")


def prepare_image(img):
    """RetinaNet 预处理（保持长宽比 1333x800）"""
    img = mmcv.imrescale(img, (1333, 800))  # 标准输入尺寸
...
with torch.no_grad():
    for img in tqdm(processed_imgs, desc='特征提取'):
        _ = model.backbone(img)  # 仅提取backbone特征
...
# 保存中间数据
output_file = os.path.join(SAVE_DIR, f'retinanet_features_{args.k}.pkl')
