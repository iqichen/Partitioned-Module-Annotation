...
# 参数解析
parser = argparse.ArgumentParser(description='使用K-means聚类拆分EfficientNet模型')
...
# 初始化配置
config_file = r'E:\Code\Partitioned Module Annotation\mmdetection\configs\efficientnet\retinanet_effb3_fpn_8xb4-crop896-1x_coco.py'
checkpoint_file = r'E:\Code\Partitioned Module Annotation\mmdetection\checkpoints\retinanet_effb3_fpn_crop896_8x4_1x_coco_20220322_234806-615a0dda.pth'
...
def group_layers_by_block(layers):
    """将卷积层按照block级别合并"""
    block_groups = {}
    # 针对EfficientNet的层级结构调整正则表达式
    block_pattern = re.compile(r'layers\.(\d+)\.(\d+)')
...
for block_key, layer_names in block_groups.items():
    # 优先选择linear_conv.conv作为代表（EfficientNet的MBConv结构）
    for name in layer_names:
        if 'linear_conv.conv' in name:
            selected_layers[block_key] = all_conv_layers[name]
            break
    else:
        # 如果没有linear_conv.conv，则选择第一个
...
def prepare_image(img):
    scaled_img = mmcv.imrescale(img, (896, 896))
    h, w = scaled_img.shape[:2]
    crop_size = min(h, w)
    y_start = (h - crop_size) // 2
    x_start = (w - crop_size) // 2
    cropped_img = mmcv.imcrop(scaled_img, bboxes=np.array([x_start, y_start, x_start + crop_size, y_start + crop_size]))

    img_tensor = torch.from_numpy(cropped_img).permute(2, 0, 1).unsqueeze(0).cuda().float()
    img_tensor = (img_tensor / 255.0 -
                  torch.tensor([0.485, 0.456, 0.406], device='cuda').view(1, 3, 1, 1)) / \
...
for path in tqdm(img_paths, desc='预处理图片'):
    ...
    original_imgs.append(mmcv.imresize(img, (896, 896)))
...
# 保存中间数据
output_file = os.path.join(SAVE_DIR, f'efficientnet_features_{args.k}.pkl')
