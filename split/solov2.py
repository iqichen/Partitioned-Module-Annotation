...
# 参数解析
parser = argparse.ArgumentParser(description='使用K-means聚类拆分SOLOv2模型')
...
config_file = r'E:\Code\Partitioned Module Annotation\mmdetection\configs\solov2\solov2_r50_fpn_1x_coco.py'
checkpoint_file = r'E:\Code\Partitioned Module Annotation\mmdetection\checkpoints\solov2_r50_fpn_1x_coco_20220512_125858-a357fa23.pth'
...
# 获取模型中的所有卷积层
all_conv_layers = get_all_conv_layers(model.neck)
...
def group_layers_by_block(layers):
    """将卷积层按照block级别合并"""
    block_groups = {}
    # 针对SOLOv2的层级结构调整正则表达式
    block_pattern = re.compile(r'lateral_convs\.(\d+)')
...
for layer_name, layer in selected_layers.items():
    try:
        ...
        if orig_layer_name:
            handle = layer.register_forward_hook(hook.hook_func)
...
def prepare_image(img):
    img = mmcv.imrescale(img, (1333, 800))
    img = mmcv.imconvert(img, 'bgr', 'rgb')
    img = (img.astype(np.float32) / 255.0 -
           np.array([123.675, 116.28, 103.53])) / np.array([58.395, 57.12, 57.375])
...
for path in tqdm(img_paths, desc='预处理图片'):
    ...
    original_imgs.append(mmcv.imresize(img, (1333, 800)))
    processed_imgs.append(prepare_image(img))

with torch.no_grad():
    for img in tqdm(processed_imgs, desc='特征提取'):
        _ = model.extract_feat(img)

# 收集特征数据
module_features = {mod: hook.features[:10] for mod, (hook, _) in hooks.items()}
...
intermediate_data = {
    ...
    'original_imgs': [cv2.cvtColor(img, cv2.COLOR_BGR2RGB) for img in original_imgs],
...
# 保存中间数据
output_file = os.path.join(SAVE_DIR, f'solov2_features_{args.k}.pkl')
