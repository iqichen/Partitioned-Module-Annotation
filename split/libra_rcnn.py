...
# 参数解析
parser = argparse.ArgumentParser(description='使用K-means聚类拆分Libra RCNN模型')
...
# 初始化配置
config_file = r'E:\Code\Partitioned Module Annotation\mmdetection\configs\libra_rcnn\libra-faster-rcnn_r50_fpn_1x_coco.py'
checkpoint_file = r'E:\Code\Partitioned Module Annotation\mmdetection\checkpoints\libra_faster_rcnn_r50_fpn_1x_coco_20200130-3afee3a9.pth'
...
def prepare_image(img):
    """Libra R-CNN 预处理（保持长宽比 800x1333）"""
    img = mmcv.imrescale(img, (1333, 800))  # 短边800，长边不超过1333
...
# 保存中间数据
output_file = os.path.join(SAVE_DIR, f'libra_rcnn_features_{args.k}.pkl')
